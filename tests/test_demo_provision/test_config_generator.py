...
class TestGenerateProjectMap:
    def test_produces_valid_json(self) -> None:
        result = generate_project_map(JIRA_PROJECT_KEY, GITLAB_PROJECT_PATH)

        parsed = json.loads(result)
        assert JIRA_PROJECT_KEY in parsed
        assert parsed[JIRA_PROJECT_KEY]["gitlab_project"] == GITLAB_PROJECT_PATH
        assert parsed[JIRA_PROJECT_KEY]["target_branch"] == "main"

    def test_custom_target_branch(self) -> None:
        result = generate_project_map(JIRA_PROJECT_KEY, GITLAB_PROJECT_PATH, "develop")

        parsed = json.loads(result)
        assert parsed[JIRA_PROJECT_KEY]["target_branch"] == "develop"
